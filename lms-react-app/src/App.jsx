...
const VIEW_MAP = {
  dashboard: Dashboard, loanmgmt: LoanManagement, screens: AppScreens,
  codeviewer: CodeViewer, calltree: CallTree, process: ProcessFlow,
  dependency: Dependency, impact: ImpactAnalysis, crud: CrudReport,
  dbconn: DbConnections, testcases: TestCases, decomp: Decomposition,
  bizrules: BusinessRules, reqdoc: ReqDocument, testexec: TestExecution,
...
export default function App() {
  ...
  function handleLogin(r) { setRole(r); setCurrentView('dashboard'); }
  function handleLogout() { setRole(null); setCurrentView('dashboard'); }
  function handleNavigate(v) { if (VIEW_MAP[v]) setCurrentView(v); }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', background: 'var(--bg)' }}>
          <ViewComponent role={role} onNavigate={handleNavigate} onToast={msg => setToast(msg)} />
        </main>
