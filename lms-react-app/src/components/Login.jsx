  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#0d1117', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '40px 48px', width: 420, textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 28 }}>Loan Management System — Modernization Analysis Platform<br />HP NonStop C++ / COBOL → .NET 8 / Azure</div>
        <div style={{ fontSize: 11, color: 'var(--muted)', textAlign: 'left', marginBottom: 6 }}>Select your role to begin:</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
          {ROLES.map(r => (
            <div key={r.id} onClick={() => setSelected(r.id)} style={{ background: selected === r.id ? '#1f3a5f66' : 'var(--surface2)', border: `2px solid ${selected === r.id ? 'var(--blue)' : 'var(--border)'}`, borderRadius: 8, padding: '14px 12px', cursor: 'pointer', textAlign: 'left', transition: '.15s' }}>
        ...
        <button onClick={() => onLogin(selected)} style={{ width: '100%', padding: 12, background: 'var(--blue)', color: '#fff', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Enter Platform →</button>
      </div>
    </div>
  );
}
